...
SDRPP_MOD_INFO{
    ...
    /* Description:     */ "Frequency manager module for Aster",
    /* Author:          */ "Ryzerth;Zimm",
    /* Version:         */ 0, 3, 0,
    /* Max instances    */ 1
};
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        if (selectedNames.size() != 1 && _this->selectedListName != "") { style::beginDisabled(); }
        if (ImGui::Button(("OK ##_freq_mgr_apply_" + _this->name).c_str(), ImVec2(menuWidth, 0))) {
    ...
    static void fftInput(ImGui::WaterFall::InputHandlerArgs args, void* ctx) {
        ...
        ImGui::Text("Смуга: %s", utils::formatFreq(hoveredBookmark.bookmark.bandwidth).c_str());
