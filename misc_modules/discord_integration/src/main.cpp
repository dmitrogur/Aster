...
SDRPP_MOD_INFO{
    ...
    /* Description:     */ "Discord Rich Presence module for Aster",
    ...
    void updatePresence() {
        ...
        if (selectedFreq != lastFreq || mode != lastMode) {
            ...
            if (selectedFreq >= 1000000.0) {
                sprintf(freq, "%.3lfМГц %s", selectedFreq / 1000000.0, mode);
            }
            else if (selectedFreq >= 1000.0) {
                sprintf(freq, "%.3lfKГц %s", selectedFreq / 1000.0, mode);
            }
            else {
                sprintf(freq, "%.3lfГц %s", selectedFreq, mode);
            }
