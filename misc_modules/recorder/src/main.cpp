...
//        sampleTypes.define(wav::SAMP_TYPE_FLOAT32, "Float32", wav::SAMP_TYPE_FLOAT32);
        ...
        sampleTypeId = sampleTypes.valueId(wav::SAMP_TYPE_UINT8);
    ...
    std::string genWavFileName(const double current,  const int _mode) {
        ...
        flog::warn("radio. genWavFileName: {0}, ms ={1}", templ, std::to_string(ms.count()));
    ...
    void start() {
        ...
        if(currWavFile=="") {
            ...
            if(gui::waterfall.selectedVFO!="") {
                ...
                flog::warn("radio. getModuleName: {0}", gui::waterfall.selectedVFO);
            }

            // core::modComManager.callInterface(gui::waterfall.selectedVFO, RADIO_IFACE_CMD_GET_MODE, NULL, &_mode);                                

            currWavFile = genWavFileName(current, _mode);
        }    
...
//        std::string expandedPath = expandString(folderSelect.path + "/" + genFileName(nameTemplate, type, vfoName) + extension);
        std::string expandedPath = expandString(folderSelect.path + "/" + currWavFile); //genWavFileName(vfoName) + extension);
...
//        flog::error("start recording ={0}, name{1}", recording, name);
        recording = true;
    }
    ...
    void stop() {
        ...
        recording = false;
//        flog::error("stop recording ={0}, name{1}", recording, name);
...
private:
    static void menuHandler(void* ctx) {
        ...
        if (ImGui::RadioButton(CONCAT("СМО##_recorder_mode_", _this->name), _this->recMode == RECORDER_MODE_BASEBAND)) {
        ...
        ImGui::NextColumn();
        if (ImGui::RadioButton(CONCAT("Аудіо##_recorder_mode_", _this->name), _this->recMode == RECORDER_MODE_AUDIO)) {
...
/*
            ...
            if (_this->recording) { style::endDisabled(); }
*/
            if (ImGui::Checkbox(CONCAT("Ігнорувати паузи##_recorder_ignore_silence_", _this->name), &_this->ignoreSilence)) {
                config.acquire();
        ...
        if (!_this->recording) {
            if (ImGui::Button(CONCAT("Запис##_recorder_rec_", _this->name), ImVec2(menuWidth, 0))) {
                _this->start();
            }
            ImGui::TextColored(ImGui::GetStyleColorVec4(ImGuiCol_Text), "Очікування --:--:--");
        }
        else {
            if (ImGui::Button(CONCAT("Стоп##_recorder_rec_", _this->name), ImVec2(menuWidth, 0))) {
                _this->stop();
            }
            ...
            if (_this->ignoreSilence && _this->ignoringSilence) {
                ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "Пауза %02d:%02d:%02d", dtm->tm_hour, dtm->tm_min, dtm->tm_sec);
            }
            else {
    ...
    void selectStream(std::string name) {
        ...
        startAudioPath();
//        flog::warn("++++ TRACE! selectStream {0}", name);
    ...
    static void moduleInterfaceHandler(int code, void* in, void* out, void* ctx) {
        ...
        if (code == RECORDER_IFACE_CMD_GET_MODE) {
            ...
            *_out = _this->recMode;
            ...
            _this->currWavFile = s;
            flog::info("RECORDER_IFACE_CMD_START: name {0}, _this->recording {1},  file = {2}", _this->name, _this->recording, _this->currWavFile);
            if (!_this->recording) { _this->start(); }
        }
        else if (code == RECORDER_IFACE_CMD_STOP) {
            flog::info("RECORDER_IFACE_CMD_STOP: name {0}, _this->recording {1}", _this->name, _this->recording);
            if (_this->recording) { _this->stop(); }
        }
        else if (code == RECORDER_IFACE_CMD_SET_STREAM) {
            // _this->streamId =2;
            int* _in = (int*)in;
            _this->streamId =  std::clamp<int>(*_in, 0, 100);
            // _this->selectedStreamName = _this->audioStreams.value(_this->streamId);
            // _this->ignoreSilence = true;
            _this->selectStream(_this->audioStreams.value(_this->streamId));
            config.acquire();
            config.conf[_this->name]["audioStream"] = _this->audioStreams.key(_this->streamId);
 flog::info(" TRACE _this->streamId ={0}, _this->audioStreams.value(_this->streamId)= {1} ", _this->streamId, _this->audioStreams.value(_this->streamId));                    
