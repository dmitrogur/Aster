...
SinkManager::SinkManager() {
    ...
    registerSinkProvider("Вимк.", prov);
//    core::modComManager.registerInterface("sink", "None", moduleInterfaceHandler, this);
}
...
void SinkManager::registerStream(std::string name, SinkManager::Stream* stream, bool fisrt) {
    ...
    provider = providers["Вимк."];

    stream->sink = provider.create(stream, name, provider.ctx);
    stream->providerId = std::distance(providerNames.begin(), std::find(providerNames.begin(), providerNames.end(), "Вимк."));
    stream->providerName = "Вимк.";

    streams[name] = stream;
    streamNames.push_back(name);

    // flog::info("SinkManager::registerStream.... {0}", name);
    
    core::configManager.acquire();
    stream->volumeAjust.setMuted(false);
    stream->setVolume(0.5);
    setStreamSink(name, "Аудіо"); //providerNames[1]
...
//    flog::info("TRACE.... showVolumeSlider");
...
//    flog::info("TRACE.... saveStreamConfig({0});", name);
    ...
    if (ImGui::SliderFloat((prefix + name).c_str(), &stream->guiVolume, 0.0f, 1.0f, "")) {
        ...
        core::configManager.acquire();
        saveStreamConfig(name);
//        flog::info("TRACE.... SliderFloat({0});", name);
        core::configManager.release(true);
    }
    if (sameLine) { ImGui::SetCursorPosY(ypos); }
...
void SinkManager::loadStreamConfig(std::string name) {
    flog::info("SinkManager::loadStreamConfig.... {0}", name);
    ...
    // flog::info("2 SinkManager::loadStreamConfig.... {0}", name);
}
...
void SinkManager::saveStreamConfig(std::string name) {
    ...
    flog::info("SinkManager::saveStreamConfig.... {0}", name);
}
...
void SinkManager::loadSinksFromConfig() {
    for (auto const& [name, stream] : streams) {
        ...
        flog::info("SinkManager::loadSinksFromConfig... {0}", name);        
    }
}
...
/*
// static 
void SinkManager::moduleInterfaceHandler(int code, void* in, void* out, void* ctx) {
    SinkManager* _this = (SinkManager*) ctx;
    flog::info(" SinkManager::moduleInterfaceHandler....");
