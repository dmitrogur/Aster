...
//    gui::menu.registerEntry("Частотний план", bandplanmenu::draw, NULL);
    gui::menu.registerEntry("Дисплей", displaymenu::draw, NULL);
    ...
    flog::info("frequency {0}, old_freq {1}", frequency, old_freq);
...
void MainWindow::vfoAddedHandler(VFOManager::VFO* vfo, void* ctx) {
    ...
    flog::info("____name {0}, setCenterOffset {1} and {2}. viewBW {3}", name, newOffset, offset, viewBW);
...
void MainWindow::draw() {
    ...
    bool _changeOffset = true; 
    if(gui::freqSelect.old_freq==0 || tuningMode != tuner::TUNER_MODE_CENTER) {
        _changeOffset = false;
    }
    ...
    if (vfo != NULL) {
        if (vfo->centerOffsetChanged) {
           flog::info("1____ vfo->centerOffsetChanged {0}, freq = {1}, generalOffset = {2} ", vfo->centerOffsetChanged, gui::waterfall.getCenterFrequency(),  vfo->generalOffset);
            ...
            if(_changeOffset==true) {
                _changeOffset=false;
                double freq_offset = gui::freqSelect.old_freq - gui::waterfall.getCenterFrequency();
                for (auto const& [name, vfo] : gui::waterfall.vfos) {
                    if(name!="Канал приймання") {
                        double generalOffset = vfo->generalOffset + freq_offset;
                        flog::info("tuning ({0}), old generalOffset {1}, new generalOffset {2}", name,  vfo->generalOffset, generalOffset);
    ...
    if (gui::waterfall.selectedVFOChanged) {
        flog::info("2____ gui::freqSelect.setFrequency {0}, generalOffset = {1} ", gui::waterfall.getCenterFrequency(),  vfo->generalOffset);
    ...
    if (gui::freqSelect.frequencyChanged) {
        ...
        flog::info("3____gui::freqSelect.frequencyChanged {0}, generalOffset = {1} ", gui::waterfall.getCenterFrequency(),  vfo->generalOffset);
    ...
    if (gui::waterfall.centerFreqMoved) {
        gui::waterfall.centerFreqMoved = false;
        flog::info("4____gui::waterfall.centerFreqMoved {0}, generalOffset = {1} ", gui::waterfall.getCenterFrequency(),  vfo->generalOffset);
    ...
    if (tuningMode == tuner::TUNER_MODE_CENTER) {
        ...
        if (ImGui::ImageButton(icons::CENTER_TUNING, btnSize, ImVec2(0, 0), ImVec2(1, 1), 5, ImVec4(0, 0, 0, 0), textCol)) {
            /*
            tuningMode = tuner::TUNER_MODE_CENTER;// tuner::TUNER_MODE_NORMAL;
            tuner::tune(tuner::TUNER_MODE_CENTER, gui::waterfall.selectedVFO, gui::freqSelect.frequency);
            gui::waterfall.VFOMoveSingleClick = true; // false;
            core::configManager.acquire();
            core::configManager.conf["centerTuning"] = true;// false;
            core::configManager.release(true);
            */
        }
        ImGui::PopID();
...
*/        
    ...
    ImGui::SetCursorPosX((ImGui::GetWindowSize().x / 2.0) - (ImGui::CalcTextSize("Масш.").x / 2.0));
    ...
    ImGui::SetCursorPosX((ImGui::GetWindowSize().x / 2.0) - (ImGui::CalcTextSize("Макс.").x / 2.0));
    ...
    ImGui::SetCursorPosX((ImGui::GetWindowSize().x / 2.0) - (ImGui::CalcTextSize("Мін.").x / 2.0));
