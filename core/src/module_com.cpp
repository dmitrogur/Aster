...
std::string ModuleComManager::getModuleName(std::string name) {
    std::lock_guard<std::mutex> lck(mtx);
    if (interfaces.find(name) == interfaces.end()) {
        flog::error("getModuleName. Tried to call unknown module interface: {0}", name);
        return "";
    }
    flog::warn("getModuleName: {0}", name);
...
bool ModuleComManager::callInterface(std::string name, int code, void* in, void* out) {
    std::lock_guard<std::mutex> lck(mtx);
    if (interfaces.find(name) == interfaces.end()) {
        flog::error("callInterface. Tried to call unknown module interface: {0}", name);
        return false;
    }
