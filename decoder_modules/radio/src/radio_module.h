...
public:
    RadioModule(std::string name) {
        ...
        // Initialize the VFO
        vfo = sigpath::vfoManager.createVFO(name, ImGui::WaterfallVFO::REF_CENTER, 0, 200000, 200000, 1000, 220000, false);
        ...
        sigpath::sinkManager.registerStream(name, &stream, true);
...
// 2,5; 5; 10; 12,5; 20; 25; 30 или 50 кГц.
        ...
        for (int i = 0; i < bandwidthsList.size(); i++) {
            ...
            if (bandwidthsList[i] == bandwidth) {
                bandwidthId = i;
                flog::info("TRACE. bandwidth 4 = {0}!", bandwidth);
                break;
            }
        }
...
private:
    static void menuHandler(void* ctx) {
        ...
        if (!_this->bandwidthLocked) {
            ImGui::LeftLabel("Смуга");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
            if (ImGui::Combo(("##_radio_bw_" + _this->name).c_str(), &_this->bandwidthId, _this->bandwidthsListTxt.c_str())) {
                // _this->bandwidth = _this->bandwidthsList[_this->bandwidthId];
                // flog::info("TRACE. bandwidth 3 = {0}!", _this->bandwidthsList[1]);

                // _this->bandwidth = _this->bandwidthsList[_this->bandwidthId];
                // _this->bandwidth = _this->bandwidthsList[1];
                // std::clamp<uint32_t>(_this->bandwidthsList[_this->bandwidthId], 1000.0, _this->maxBandwidth);
                _this->setBandwidth(_this->bandwidthsList[_this->bandwidthId]);
                flog::info("TRACE. bandwidth = {0}, {1}!", _this->bandwidth, _this->bandwidthsList[_this->bandwidthId]);
            }
            // _this->minBandwidth
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(("##_radio_snap_" + _this->name).c_str(), &_this->snapIntervalId, _this->snapintervalsListTxt.c_str())) {
...
*/        
        ...
        if (_this->deempAllowed) {
            ImGui::LeftLabel("Послаблення ВЧ (для NFM)");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        ...
        // Squelch
        if (ImGui::Checkbox(("Поріг ШП##_radio_sqelch_ena_" + _this->name).c_str(), &_this->squelchEnabled)) {
    ...
    demod::Demodulator* instantiateDemod(DemodID id) {
        ...
        for (int i = 0; i < bandwidthsList.size(); i++) {
            if (bandwidthsList[i] == bandwidth) {
                bandwidthId = i;
                flog::info("TRACE. bandwidth 1 = {0}!", bandwidth);
                break;
            }
        }
    ...
    void selectDemodByID(DemodID id) {
        ...
        flog::warn("Demod switch took {0} мкс", (int64_t)((std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime)).count()));
    }
    ...
    void selectDemod(demod::Demodulator* demod) {
        ...
        // Configure VFO
        flog::info("TRACE. bandwidth 3 = {0}", bandwidth);        
        if (vfo) {
        ...
        if(bandwidthId==0) snapIntervalId  = snapintervalsList.size();
    ...
    void setBandwidth(double bw) {
        ...
        bw = std::clamp<double>(bw, 1000, maxBandwidth);
    ...
    std::string snapintervalsListTxt;
    int snapIntervalId=0;
    std::vector<uint32_t> bandwidthsList;
    std::string bandwidthsListTxt;
    int bandwidthId=0;
