...
SDRPP_MOD_INFO{
    ...
    /* Description:     */ "Meteor demodulator for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ -1
};
...
private:
    static void menuHandler(void* ctx) {
        ...
        if (_this->recording) {
            if (ImGui::Button(CONCAT("Стоп##meteor_rec_", _this->name), ImVec2(menuWidth, 0))) {
                _this->stopRecording();
            }
            ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Запис %.2fMB", (float)_this->dataWritten / 1000000.0f);
        }
        else {
            if (ImGui::Button(CONCAT("Запис##meteor_rec_", _this->name), ImVec2(menuWidth, 0))) {
                _this->startRecording();
            }
            ImGui::TextUnformatted("Очікування --.--MB");
        }
