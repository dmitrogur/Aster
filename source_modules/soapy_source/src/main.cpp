...
SDRPP_MOD_INFO{
    /* Name:            */ "soapy_source",
    /* Description:     */ "SoapySDR source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 5,
    /* Max instances    */ 1
};
...
private:
    ...
    void selectDevice(std::string name) {
        ...
        for (auto bwr : bandwidthRange) {
            ...
            if (bw > 1.0e3 && bw <= 1.0e6) {
                txtBwList += to_string_with_precision((bw / 1.0e3), 2) + " кГц";
            }
            else if (bw > 1.0e6) {
                txtBwList += to_string_with_precision((bw / 1.0e6), 2) + " МГц";
            }
            else {
        ...
        for (double sr : sampleRates) {
            if (sr > 1.0e3 && sr <= 1.0e6) {
                txtSrList += to_string_with_precision((sr / 1.0e3), 2) + " кГц";
            }
            else if (sr > 1.0e6) {
                txtSrList += to_string_with_precision((sr / 1.0e6), 2) + " МГц";
            }
            else {
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->devId < 0) {
            SmGui::FillWidth();
            SmGui::ForceSync();
            if (SmGui::Button(CONCAT("Оновити##_dev_select_", _this->name))) {
                _this->refresh();
