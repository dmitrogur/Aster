...
public:
    AirspySourceModule(std::string name) {
        ...
        core::modComManager.registerInterface("Airspy", name, moduleInterfaceHandler, this);
...
//        flog::info("AirspySourceModule '{0}': Tune: {1}!", _this->name, freq);
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Button(CONCAT("Оновити##_airspy_refr_", _this->name))) {
            _this->refresh();
            config.acquire();
            std::string devSerial = config.conf["device"];
            config.release();
        ...
        SmGui::ForceSync();
        if (SmGui::RadioButton(CONCAT("Чутлив.##_airspy_gm_", _this->name), _this->gainMode == 0)) {
            _this->gainMode = 0;
            if (_this->running) {
                airspy_set_lna_agc(_this->openDev, 0);
                airspy_set_mixer_agc(_this->openDev, 0);
        ...
        // Bias T
        if (SmGui::Checkbox(CONCAT("Живл. МШП##_airspy_", _this->name), &_this->biasT)) {
            if (_this->running) {
                airspy_set_rf_bias(_this->openDev, _this->biasT);
            }
            if (_this->selectedSerStr != "") {
...
///           flog::info("IRSPY_IFACE_CMD_GET_RECORDING select= {0}, _this->running {1}", _this->select, _this->running);              
        ...
        } else if(code == AIRSPY_IFACE_CMD_START) {
            // _this->start();
        } else if(code == AIRSPY_IFACE_NAME_SER_NOM ) {
            std::string* _out = (std::string*)out;
            *_out = "test2"; //_this->selectedSerStr;
        }
