...
SDRPP_MOD_INFO{
    ...
    /* Description:     */ "SpyServer source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (!connected && SmGui::Button("З'єднати##spyserver_source")) {
            _this->tryConnect();
        }
        else if (connected && SmGui::Button("Відключити##spyserver_source")) {
            _this->client->close();
        }
