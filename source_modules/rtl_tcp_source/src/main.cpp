...
SDRPP_MOD_INFO{
    /* Name:            */ "rtl_tcp_source",
    /* Description:     */ "RTL-TCP source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 1, 1, 0,
    /* Max instances    */ 1
};
...
public:
    RTLTCPSourceModule(std::string name) {
        ...
        // Define samplerates
        samplerates.define(250e3, "250KГц", 250e3);
        samplerates.define(1.024e6, "1.024МГц", 1.024e6);
        samplerates.define(1.536e6, "1.536МГц", 1.536e6);
        samplerates.define(1.792e6, "1.792МГц", 1.792e6);
        samplerates.define(1.92e6, "1.92МГц", 1.92e6);
        samplerates.define(2.048e6, "2.048МГц", 2.048e6);
        samplerates.define(2.16e6, "2.16МГц", 2.16e6);
        samplerates.define(2.4e6, "2.4МГц", 2.4e6);
        samplerates.define(2.56e6, "2.56МГц", 2.56e6);
        samplerates.define(2.88e6, "2.88МГц", 2.88e6);
        samplerates.define(3.2e6, "3.2МГц", 3.2e6);
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##_biast_select_", _this->name), &_this->biasTee)) {
            if (_this->running) {
                _this->client->setBiasTee(_this->biasTee);
            }
            config.acquire();
