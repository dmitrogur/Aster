...
SDRPP_MOD_INFO{
    /* Name:            */ "hackrf_source",
    /* Description:     */ "HackRF source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
const char* sampleRatesTxt = "20MГц\00016MГц\00010MГц\0008MГц\0005MГц\0004MГц\0002MГц\000";
...
const char* bandwidthsTxt = "1.75МГц\0"
                            "2.5МГц\0"
                            "3.5МГц\0"
                            "5МГц\0"
                            "5.5МГц\0"
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Button(CONCAT("Оновити##_hackrf_refr_", _this->name))) {
            _this->refresh();
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##_hackrf_bt_", _this->name), &_this->biasT)) {
            if (_this->running) {
                hackrf_set_antenna_enable(_this->openDev, _this->biasT);
            }
            config.acquire();
