...
SDRPP_MOD_INFO{
    /* Name:            */ "sdrplay_source",
    /* Description:     */ "SDRplay source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->ifModeId == 0) {
            ...
            SmGui::FillWidth();
            SmGui::ForceSync();
            if (SmGui::Button(CONCAT("Оновити##sdrplay_refresh", _this->name))) {
                _this->refresh();
                _this->selectByName(_this->selectedName);
            }

            SmGui::LeftLabel("Смуга");
            SmGui::FillWidth();
    ...
    bool agcParamMenu(bool& valid) {
        ...
        if (SmGui::BeginPopup("Edit##sdrplay_source_edit_agc_params_", ImGuiWindowFlags_NoResize)) {
            if (SmGui::BeginTable(("sdrplay_source_agc_param_tbl" + name).c_str(), 2)) {
                ...
                SmGui::TableSetColumnIndex(1);
                SmGui::SetNextItemWidth(100);
                SmGui::InputInt("дБ##sdrplay_source_agc_decay_thresh", &_agcDecayThreshold);
    ...
    void RSP1AMenu() {
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##sdrplay_rsp1a_biast", name), &rsp1a_biasT)) {
            if (running) {
    ...
    void RSP2Menu() {
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##sdrplay_rsp2_biast", name), &rsp2_biasT)) {
            if (running) {
    ...
    void RSPduoMenu() {
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##sdrplay_rspduo_biast", name), &rspduo_biasT)) {
            if (running) {
    ...
    void RSPdxMenu() {
        ...
        if (SmGui::Checkbox(CONCAT("Живл. МШП##sdrplay_rspdx_biast", name), &rspdx_biasT)) {
            if (running) {
