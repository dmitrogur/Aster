...
public:
    ...
    void menuHandler() {
        ...
        ImGui::LeftLabel("Частота дискретизации");
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        ...
        if (ImGui::Checkbox(CONCAT("Стерео##_network_sink_stereo_", _streamName), &stereo)) {
            stop();
            start();
            config.acquire();
        ...
        if (listening && ImGui::Button(CONCAT("Старт##_network_sink_stop_", _streamName), ImVec2(menuWidth, 0))) {
            stopServer();
            config.acquire();
            config.conf[_streamName]["listening"] = false;
            config.release(true);
        }
        else if (!listening && ImGui::Button(CONCAT("Старт##_network_sink_stop_", _streamName), ImVec2(menuWidth, 0))) {
            startServer();
            config.acquire();
        ...
        ImGui::SameLine();
        if (conn && conn->isOpen()) {
            ImGui::TextColored(ImVec4(0.0, 1.0, 0.0, 1.0), (modeId == SINK_MODE_TCP) ? "Соединение" : "Отправка");
        }
        else if (listening) {
            ImGui::TextColored(ImVec4(1.0, 1.0, 0.0, 1.0), "Прослушивание");
        }
        else {
