...
std::vector<RtAudio::DeviceInfo> gendevList;
...
public:
    AudioSink(SinkManager::Stream* stream, std::string streamName) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            gendevList.push_back(info);
            gendevList[i].preferredSampleRate = 8000;
...
public:
    AudioSinkModule(std::string name) {
        ...
        core::modComManager.registerInterface("audio_sink", name, moduleInterfaceHandler, this);
        flog::info("!!! registerInterface, name = {0} ",  name);
    }
...
private:
    ...
    static void moduleInterfaceHandler(int code, void* in, void* out, void* ctx) {
        ...
        // flog::info("moduleInterfaceHandler, name = {0}, list = {1} ",  _this->streamName, _this->txtDevList);
       // std::lock_guard lck(_this->recMtx);
       
        if (code == AUDIO_SINK_GET_DEVLIST) {
            std::string* _out = (std::string*)out;
            *_out = txtDevList;
        } else if (code == AUDIO_SINK_CMD_SET_DEV) {
            struct setDev{
                std::string sndname; 
                int id;
            } psetDev;
            psetDev =  *(static_cast<setDev *>(in)); 
            flog::info("!!! rwork_devId={0}, psetDev.sndname = {1}", psetDev.id, psetDev.sndname);

            config.acquire();
            config.conf[psetDev.sndname]["device"] = gendevList[psetDev.id].name;
            /*
            if (!config.conf[psetDev.sndname]["devices"].contains(devList[psetDev.id].name)) {
                config.conf[psetDev.sndname]["devices"][devList[psetDev.id].name] = devList[psetDev.id].preferredSampleRate;
            }
            */
            config.release(true);
        }
